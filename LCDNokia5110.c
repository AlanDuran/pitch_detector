...
void LCDNokia_init(void) {
  ...
  ///Configure control pins
	...
	LCDNokia_writeByte(LCD_CMD, EXT_CMD); ///Tell LCD that extended commands follow
	LCDNokia_writeByte(LCD_CMD, CONTRAST); ///Set LCD Vop (Contrast): Try 0xB1(good @ 3.3V) or 0xBF if your display is too dark
	LCDNokia_writeByte(LCD_CMD, TEMPC); ///Set Temp coefficent
	LCDNokia_writeByte(LCD_CMD, BIAS); ///LCD bias mode 1:48: Try 0x13 or 0x14

	LCDNokia_writeByte(LCD_CMD, CONTROL); ///We must send 0x20 before modifying the display control mode
	LCDNokia_writeByte(LCD_CMD, NORMAL_MODE); ///Set display control, normal mode. 0x0D for inverse
}

void LCDNokia_bitmap(const uint8* my_array){
	uint16 index=0;
	/**Writes the image bitmap on every position of display*/
...
void LCDNokia_clear(void) {
	uint16 index = 0;
	/**Writes 0 over every display position*/
	for (index = 0 ; index < (LCD_X * LCD_Y / 8) ; index++)
...
void LCDNokia_gotoXY(uint8 x, uint8 y) {
	/**Sends command to move cursor to a given position*/
